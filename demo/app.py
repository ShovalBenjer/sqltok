...
DEFAULT_DDL = """CREATE TABLE customers (id INTEGER PRIMARY KEY, name TEXT,
    city_id INTEGER, created_at TEXT);
...
CREATE TABLE reviews (id INTEGER PRIMARY KEY, product_id INTEGER,
    customer_id INTEGER, stars INTEGER, body TEXT);
...
if go:
    ...
    ctx = mgr.build_context(
        question, token_budget=budget, include_sample_rows=False, fk_expand=fk_expand
    )
    ...
    st.caption(
        f"Selector: {ctx.selector} · budget {ctx.budget} "
        "· every token measured with tiktoken."
    )
