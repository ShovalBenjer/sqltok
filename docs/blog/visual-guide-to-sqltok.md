...
These figures are deterministic. Token reduction looks modest because BIRD schemas are small; the savings grow with schema size, while recall is the size-independent correctness metric. The remaining question, whether the model answers correctly at the lower token count, is answered by running the official BIRD execution-accuracy script with a real model, which the repository supports directly.
